...
class LandscapeNode
{
    ...
    float GetAltitudeAtLocation(float posX, float posZ)
    {
        return Mathf.PerlinNoise(posX / Landscape.noiseScale, posZ / Landscape.noiseScale) * 500;
    }
    ...
    void createMesh()
    {
        ...
        for (int x = 0; x < VerticesPerChunk; ++x)
        {
            for (int y = 0; y < VerticesPerChunk; ++y)
            {
                ...
                v_position[VertexIndex] = new Vector3(l_PosX, GetAltitudeAtLocation(l_PosX, l_posZ), l_posZ);
    ...
    private int computeDesiredLODLevel()
    {
        ...
        cameraGroundLocation.y -= GetAltitudeAtLocation(Landscape.GetCameraPosition().x, Landscape.GetCameraPosition().z);
