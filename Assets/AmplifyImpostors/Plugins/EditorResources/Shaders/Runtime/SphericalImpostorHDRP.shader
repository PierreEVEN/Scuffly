	...
	SubShader
	{
		...
		Pass
        {
			...
			void GetSurfaceAndBuiltinData(Features features, GlobalSurfaceDescription surfaceDescription, FragInputs fragInputs, float3 V, inout PositionInputs posInput, out SurfaceData surfaceData, out BuiltinData builtinData)
			{
				...
				#else
					InitBuiltinData( surfaceDescription.Alpha, surfaceData.normalWS, -T2W(fragInputs, 2), fragInputs.positionRWS, fragInputs.texCoord1, fragInputs.texCoord2, builtinData );
				#endif
		...
		Pass
        {
			...
			)
			{
				...
				float4 VPASSpreviousPositionCS = mul(UNITY_MATRIX_PREV_VP, float4(previousPositionRWS, 1.0));
					...
					#endif
				#else
					float2 velocity = CalculateVelocity( VPASSpositionCS, VPASSpreviousPositionCS );
					EncodeVelocity( velocity * 0.5, outMotionVector );

					bool forceNoMotion = unity_MotionVectorsParams.y == 0.0;
					if( forceNoMotion )
						outMotionVector = float4( 0.0, 0.0, 0.0, 0.0 );
